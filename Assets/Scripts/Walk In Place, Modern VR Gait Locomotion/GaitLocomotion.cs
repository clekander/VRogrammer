...
public class GaitLocomotion : MonoBehaviour
{
    ...
    public enum GaitAlignment { Camera, Direction, Forward, Path }
    ...
    void Awake()
    {
        ...
        //automatically set to follow path if component is present
        if (GetComponent<PathCreatorRunner>())
            m_gaitAlignment = GaitAlignment.Path;
    }
    ...
    void FixedUpdate()
    {
        ...
        if (m_camera != null && m_cc != null)
        {
            ...
            // Move according to Camera.forward or an arbitrary transform
            if (m_gaitAlignment != GaitAlignment.Path)
            {
                //average directions and flatten them
